(function() {
	...
	OneEuroFilter.prototype.filter = function(x, timestamp) {
		...
		// Fixed in August 2023
		var previousX = this.x.lastEstimate;
	...
	function JitterEnergyFilter(magnitudeThreshold, energyGain, decayFactor, fcMin, fcMax, energyCeiling) {
		...
		this.xPrevRaw = undefined;
		this.dPrev = 0;
		this.outputPrev = undefined;
		this.E = 0;
		this.lastTime = undefined;
	...
	JitterEnergyFilter.prototype.filter = function(x, timestamp) {
		if(this.xPrevRaw === undefined) {
			this.xPrevRaw = x;
			this.outputPrev = x;
			this.lastTime = timestamp;
			return x;
		}
		var dt = (timestamp !== undefined && this.lastTime !== undefined) ? Math.max(timestamp - this.lastTime, 1e-8) : 1.0 / 60.0;
		this.lastTime = timestamp;

		var d = x - this.xPrevRaw;
		this.E *= this.decayFactor;

		var mag = Math.abs(d);
		if(mag < this.magnitudeThreshold) {
			var dot = d * this.dPrev;
			if(dot < 0.0) {
				this.E += (-dot) * this.energyGain;
			}
		}
		...
		var alpha = this.alpha(fc, dt);
		var output = alpha * x + (1.0 - alpha) * this.outputPrev;
	...
	window.Filters = {
		...
		JitterEnergyFilter: JitterEnergyFilter
	};
})();